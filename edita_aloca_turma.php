...
//echo ('<h3>'.$op.' '.$t.' '.$p. '</h3>');
	...
	if($op == "Remover"){
		$b -> query("DELETE FROM horario_turma WHERE id_horario_turma='$id_ht';");
		//$b -> query("DELETE FROM turma_sala WHERE id_turma_sala='$id_ts';");
		?>
			<script>
				alert("Concluído com sucesso!");
				window.location = "interface_editar_aloca_turma.php";
			</script>
